...
def abrir_ventana_objeto(root):
    ...
    canvas = tk.Canvas(objeto_root, highlightthickness=0)
    canvas.place(relwidth=1, relheight=1)

    # Cargar y ajustar el tamaño de la imagen de fondo
    background_image = Image.open(r"C:\Users\oscar\OneDrive\Escritorio\deteccion_fallas\Fondos\fondoObjeto.jpg")
    background_image = background_image.resize((objeto_root.winfo_screenwidth(), objeto_root.winfo_screenheight()), Image.LANCZOS)
    ...
    def seleccionar_imagen(event=None):
        ...
        if file_path:
            ...
            canvas.create_image(objeto_root.winfo_screenwidth() * 3 / 4, 400, image=img_photo, anchor="center", tags="imagen")
...
# Función para mostrar el primer registro
    def funcion_inicio():
        try:
            ...
            if registro:
                ...
                text_descripcion.insert("1.0", registro[1])
                imagen_blob = registro[2]
                imagen = Image.open(io.BytesIO(imagen_blob))
                imagen = imagen.resize((300, 300), Image.LANCZOS)
                imagen_photo = ImageTk.PhotoImage(imagen)
                canvas.create_image(objeto_root.winfo_screenwidth() * 3 / 4, 400, image=imagen_photo, anchor="center", tags="imagen")
    ...
    def guardar_datos():
        ...
        if not nombre or not descripcion or not imagen_path:
    ...
    def eliminar_datos():
        ventana_eliminar = tk.Toplevel(objeto_root)
    ...
    def mostrar_menu_consultas():
        menu_consultas = Toplevel(objeto_root)
    ...
    def mostrar_consulta_individual():
        consulta_individual_window = Toplevel(objeto_root)
        ...
        def mostrar_detalle():
            ...
            mostrar_detalle_registro(id_seleccionado)
    ...
    def mostrar_detalle_registro(id_registro):
        detalle_window = Toplevel(objeto_root)
        ...
        try:
            ...
            tk.Label(detalle_window, text=f"Descripción: {registro[1]}", font=("Arial", 16)).pack(pady=10)
    ...
    def mostrar_consulta_general():
        consulta_general_window = Toplevel(objeto_root)
        ...
        tk.Label(header_frame, text="Descripción", font=("Arial", 18, "bold"), width=40, anchor="w").pack(side=tk.LEFT)
        ...
        try:
            ...
            for i, registro in enumerate(registros, start=1):
                ...
                tk.Label(frame, text=registro[1], font=("Arial", 18), width=40, anchor="w").pack(side=tk.LEFT)
    ...
    def modificar():
        ventana_modificar = tk.Toplevel(objeto_root)
        ...
        tree = ttk.Treeview(ventana_modificar, columns=("Nombre", "Descripción"), show='headings')
        ...
        def modificar_registro():
            ...
            label_descripcion = tk.Label(ventana_modificar_detalles, text="Descripción", font=("Arial", 14))
            label_descripcion.pack(pady=10)
            text_descripcion_modificar = tk.Text(ventana_modificar_detalles, font=("Arial", 14), height=10, width=30)
            text_descripcion_modificar.pack(pady=10)
            ...
            def cargar_detalles():
                try:
                    ...
                    if registro:
                        ...
                        text_descripcion_modificar.insert("1.0", registro[2])
                        imagen = Image.open(io.BytesIO(registro[3]))
                        imagen = imagen.resize((300, 300), Image.Resampling.LANCZOS)
                        imagen_tk = ImageTk.PhotoImage(imagen)
                        canvas_imagen_modificar.create_image(0, 0, anchor="nw", image=imagen_tk)
                        canvas_imagen_modificar.imagen_tk = imagen_tk
            ...
            def guardar_cambios():
                ...
                nueva_descripcion = text_descripcion_modificar.get("1.0", tk.END).strip()
    ...
    # Crear el texto "Nombre" en el canvas
    canvas.create_text(objeto_root.winfo_screenwidth() / 4, 150, text="Nombre:", font=("Times New Roman", 20, "bold"), fill="white", anchor="w")

    # Crear un cuadro de entrada para el nombre
    entry_nombre = tk.Entry(objeto_root, textvariable=nombre_var, font=("Times New Roman", 20), width=30)
    canvas.create_window(objeto_root.winfo_screenwidth() / 4, 190, window=entry_nombre, anchor="w")

    # Crear el texto "Descripción" en el canvas
    canvas.create_text(objeto_root.winfo_screenwidth() / 4, 270, text="Descripción:", font=("Times New Roman", 20, "bold"), fill="white", anchor="w")

    # Crear un cuadro de entrada para la descripción
    text_descripcion = tk.Text(objeto_root, font=("Times New Roman", 20), width=30, height=10)
    canvas.create_window(objeto_root.winfo_screenwidth() / 4, 310, window=text_descripcion, anchor="nw")

    # Crear el cuadro para mostrar la imagen con el texto "Seleccionar imagen"
    image_canvas = canvas.create_rectangle(objeto_root.winfo_screenwidth() * 3 / 4 - 150, 250, objeto_root.winfo_screenwidth() * 3 / 4 + 150, 550, fill="lightgray")
    canvas.create_text(objeto_root.winfo_screenwidth() * 3 / 4, 0, text="Seleccionar imagen", font=("Times New Roman", 20, "bold"), fill="black")
    ...
    # Crear los botones
    boton_inicio = tk.Button(objeto_root, text="Inicio", font=("Arial", 16), image=icono_inicio_photo, compound="left", padx=20, pady=10, command=funcion_inicio)
    boton_inicio.place(relx=0.36, rely=0.80, anchor="center")

    boton_atras = tk.Button(objeto_root, text="Atrás", font=("Arial", 16), image=icono_atras_photo, compound="left", padx=20, pady=10, command=funcion_atras)
    boton_atras.place(relx=0.48, rely=0.80, anchor="center")

    boton_siguiente = tk.Button(objeto_root, text="Siguiente", font=("Arial", 16), image=icono_siguiente_photo, compound="left", padx=20, pady=10, command=funcion_siguiente)
    boton_siguiente.place(relx=0.60, rely=0.80, anchor="center")

    boton_fin = tk.Button(objeto_root, text="Fin", font=("Arial", 16), image=icono_fin_photo, compound="left", padx=20, pady=10, command=funcion_fin)
    ...
    boton_altas = tk.Button(objeto_root, text="Altas", font=("Arial", 16), image=icono_altas_photo, compound="left", padx=20, pady=10, command=guardar_datos)
    ...
    boton_bajas = tk.Button(objeto_root, text="Bajas", font=("Arial", 16), image=icono_bajas_photo, compound="left", padx=20, pady=10, command=eliminar_datos)
    ...
    boton_consultas = tk.Button(objeto_root, text="Consultas", font=("Arial", 16), image=icono_consultas_photo, compound="left", padx=20, pady=10, command=mostrar_menu_consultas)
    ...
    boton_modificar = tk.Button(objeto_root, text="Modificar", font=("Arial", 16), image=icono_modificar_photo, compound="left", padx=20, pady=10, command=modificar)
    ...
    boton_salir = tk.Button(objeto_root, text="Salir", font=("Arial", 16),image=icono_regresar_photo, compound="left", padx=20, pady=10, command=regresar)
    boton_salir.place(relx=0.76, rely=0.9, anchor="center")
    
    # Iniciar el bucle principal de la interfaz
    objeto_root.mainloop()